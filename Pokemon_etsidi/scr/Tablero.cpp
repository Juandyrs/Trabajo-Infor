...
void Tablero::tableromueve(bool key[]) 
{
	...
	if (menu_hechizos_abierto == false )cursor.Cursormover(key, matriz); //PARA QUE NO SE DUPLIQUEN LAS CASILLAS RECORRIDAS CON EL HECHIZO SELECCIONADO
	...
	if (key['z'] || key['Z']) {
		...
		if (cursor.cursorllevaficha()) {
			...
			if (p->obtenertipomovimiento() == TipoMovimiento::Hechizo) {
				...
				Pokemon* poke_lanzador = matriz[f_hechicero][c_hechicero]; //INTERCAMBIO DE PIEZAS
